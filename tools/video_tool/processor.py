...
def _should_skip_conversion(video_path: str, output_format: str, options: Dict[str, Any]) -> Optional[str]:
    """1. Determinar si debe saltarse la conversión."""
    ...
    if input_format == output_format_lower and user_crf == default_crf:
        return f"{Path(video_path).name} - Ya está en {input_format.upper()}"
    return None
...
def _build_ffmpeg_command(video_path: str, output_path: Path, output_format: str, options: Dict[str, Any]) -> List[str]:
    ...
    if output_format in ['mp4', 'mkv']:
        cmd.extend(['-preset', 'medium', '-crf', str(crf)])
...
def _execute_ffmpeg(cmd: List[str], output_path: Path) -> Tuple[bool, str]:
    """3. Ejecutar ffmpeg y retornar éxito/error."""
    try:
...
def convert_video(files: List[str], output_format: str, **options) -> Dict[str, Any]:
    ...
    with Timer('video_tool.convert_video'):
        ...
        for video_path in files:
            ...
            # 1. Verificar si skip
            skip_reason = _should_skip_conversion(video_path, output_format, options)
            if skip_reason:
                skipped.append(skip_reason)
                logger.info(f"Omite: {Path(video_path).name}")
                continue
            ...
            # 3. Construir comando
            cmd = _build_ffmpeg_command(video_path, output_path, output_format, options)
            
            # 4. Ejecutar
            success, err = _execute_ffmpeg(cmd, output_path)
