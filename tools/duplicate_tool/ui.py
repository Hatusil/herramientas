...
class DuplicateToolUI(BaseToolUI):
    ...
    def _setup_ui(self):
        ...
        ctk.CTkButton(
            input_frame, text="Elegir", width=60, command=self._select_folder
        ).pack(side="left")
    ...
    def _build_options(self) -> None:
        ...
        ctk.CTkButton(
            self, text="\U0001F50D Buscar Duplicados",
            command=self._find_duplicates, height=40
        ).pack(pady=10)
        ...
        ctk.CTkButton(action_frame, text="\u2705 Seleccionar Todos", command=self._select_all_dups).pack(side="left", padx=5)
        ctk.CTkButton(action_frame, text="\u2B55 Deseleccionar Todos", command=self._deselect_all_dups).pack(side="left", padx=5)
        ctk.CTkButton(
            action_frame, text="\U0001F5D1\ufe0f Eliminar Seleccionados",
            command=self._delete_selected, fg_color="red", hover_color="darkred"
        ).pack(side="left", padx=5)

    def _select_all_dups(self) -> None:
        for path, var, cb in self.duplicate_groups:
            var.set(True)

    def _deselect_all_dups(self) -> None:
        for path, var, cb in self.duplicate_groups:
            var.set(False)
    ...
    def _find_duplicates(self) -> None:
        ...
        result = find_duplicates_by_size(folder) if method == "size" else find_duplicates_by_hash(folder, exts)
        ...
        if result["success"]:
            ...
            if not dups:
                ctk.CTkLabel(self.results_scroll, text="\u2705 No se encontraron duplicados", text_color="green").pack(pady=20)
                ...
                for key, files in dups.items():
                    if not files:
                        continue
                    group_label = f"Grupo: {key/1024:.1f} KB ({len(files)} archivos)" if method == "size" else f"Grupo: {len(files)} archivos"
            ...
            ctk.CTkLabel(self.results_scroll, text=f"\u274c Error: {result.get('error', 'Error')}", text_color="red").pack(pady=20)
    ...
    def _delete_selected(self) -> None:
        ...
        if not messagebox.askyesno("Confirmar", f"\u00bfEliminar {len(selected)} archivos seleccionados?\n\nEsta acci\u00f3n no se puede deshacer."):
            return
